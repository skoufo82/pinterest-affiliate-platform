...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  ...
  try {
    ...
    const { creatorId, productId, metadata } = body;
    ...
    if (!productId) {
      logger.warn('Missing required field: productId');
      return errorResponse(400, 'VALIDATION_ERROR', 'productId is required', requestId);
    }
    ...
    logger.error('Error tracking affiliate click', error as Error);
    return errorResponse(500, 'INTERNAL_ERROR', 'Failed to track affiliate click', requestId);
  }
};
